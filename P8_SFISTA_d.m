function [x_est, fk_iter, iter] = P8_SFISTA_d(A, b, c, x_init, f_opt, lambda, epsilon)
    ...
    alpha = alpha_h + lambda*alpha_g;
    beta = beta_h + lambda*beta_g;
            
    mu = sqrt(alpha/beta)*epsilon/(2*sqrt(alpha*beta));
    subg_Fmu = @(x) (1/mu)*A'*( A*x-b - Soft_Thresholding(A*x-b,mu)) + lambda*(1/mu)*(x-Soft_Thresholding(x,mu));
